...
/* Format is: Version 1.0 (Major.Minor) Build date: 5/27/19 (MM/DD/YY) */
...
CyFxUVCApplnInit (void)
{
    ...
    apiRetStatus = CyU3PDeviceGpioOverride (GW5_RESET_GPIO, CyTrue);
    if (apiRetStatus != 0)
    {
    ...
    /* GW5_RESET_GPIO is the GW55 reset pin */
    gpioConfig.outValue    = CyTrue;
    gpioConfig.driveLowEn  = CyTrue;
    gpioConfig.driveHighEn = CyTrue;
    gpioConfig.inputEn     = CyFalse;
    gpioConfig.intrMode    = CY_U3P_GPIO_NO_INTR;
    apiRetStatus           = CyU3PGpioSetSimpleConfig (GW5_RESET_GPIO, &gpioConfig);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    ...
    apiRetStatus = CyU3PDeviceGpioOverride (SENSOR_TRIGGER_GPIO, CyTrue);
    if (apiRetStatus != 0)
    {
        CyU3PDebugPrint (4, "GPIO Override failed, Error Code = %d\n", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    ...
    /* set SENSOR_TRIGGER_GPIO to High in order for keeps sensor streaming */
    gpioConfig.outValue    = CyTrue;
    gpioConfig.driveLowEn  = CyTrue;
    gpioConfig.driveHighEn = CyTrue;
    gpioConfig.inputEn     = CyFalse;
    gpioConfig.intrMode    = CY_U3P_GPIO_NO_INTR;
    apiRetStatus           = CyU3PGpioSetSimpleConfig (SENSOR_TRIGGER_GPIO, &gpioConfig);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (4, "GPIO Set Config Error, Error Code = %d\n", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    ...
    CyU3PDebugPrint (4, "Right before SensorReset()...\r\n");
...
CyFxUvcApplnStart()
{
    ...
    CyU3PDebugPrint (4, "Entered Application Start\r\n");
